                ...
                {activeTab === 'RECOVERY' && (
                    <div className="flex-grow overflow-y-auto pb-20 space-y-4">
                        <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl">
                            <h4 className="text-sm font-black text-blue-800 flex items-center gap-2 mb-2"><Database size={16}/> 自动备份机制 (Auto Backup)</h4>
                            <p className="text-xs text-blue-600 leading-relaxed font-medium">
                                系统会在每天 <strong>凌晨 4:00 AM</strong> 之后首次启动时，自动备份前一天的数据。<br/>
                                云端仅保留最近 <strong>7 天</strong> 的备份记录。
                            </p>
                        </div>
                        ...
                        <div className="bg-gray-50 border-2 border-dashed border-gray-300 p-6 rounded-xl flex flex-col items-center justify-center text-center hover:bg-gray-100 transition-colors cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                            ...
                            <p className="text-sm font-bold text-gray-600">点击上传备份文件 (.json)</p>
                            <p className="text-xs text-gray-400 mt-1">Manual Import (手动导入)</p>
                            <input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={handleFileUpload} />
                        </div>

                        <div className="h-px bg-gray-200 my-4"></div>
                        <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">云端备份列表 (Cloud Backups)</h4>

                        {loadingBackups ? (
                            <div className="py-10 text-center text-gray-400 font-bold flex flex-col items-center">
                                <Loader2 size={24} className="animate-spin mb-2"/> 加载备份列表...
                            </div>
                        ) : backups.length === 0 ? (
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex gap-2 mt-1">
                                                    ...
                                                    {bk.autoGenerated ? (
                                                        <span className="text-[10px] bg-purple-50 text-purple-600 px-1.5 py-0.5 rounded font-bold border border-purple-100 flex items-center gap-1"><RotateCcw size={10}/> Auto</span>
                                                    ) : (
